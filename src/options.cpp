...
Options::Options() : cli(false), cooler_boost(std::nullopt) {}
...
void Options::process_args(int argc, char **argv) {
    int option_index;
    while (true) {
        const auto opt = getopt_long(argc, argv, short_opts.data(), long_opts,
                                     &option_index);
        ...
        case 'B':
            ...
            if (std::strcmp(optarg, "ON") == 0) {
                cooler_boost = std::optional<Options::CoolerBoostState>{
                    Options::CoolerBoostState::ON};
            } else if (std::strcmp(optarg, "OFF") == 0) {
                cooler_boost = std::optional<Options::CoolerBoostState>{
                    Options::CoolerBoostState::OFF};
            } else if (std::strcmp(optarg, "TOGGLE") == 0) {
                cooler_boost = std::optional<Options::CoolerBoostState>{
                    Options::CoolerBoostState::TOGGLE};
            } else {
        ...
        case 'M':
            ...
            if (std::string(optarg) == "BALANCED") {
                user_mode = std::optional<Options::UserMode>{
                    Options::UserMode::BALANCED};
            } else if (std::string(optarg) == "PERFORMANCE") {
                user_mode = std::optional<Options::UserMode>{
                    Options::UserMode::PERFORMANCE};
            } else if (std::string(optarg) == "SILENT") {
                user_mode =
                    std::optional<Options::UserMode>{Options::UserMode::SILENT};
            } else if (std::string(optarg) == "BATTERY") {
                user_mode = std::optional<Options::UserMode>{
                    Options::UserMode::BATTERY};
            } else if (std::string(optarg) == "NEXT") {
                user_mode =
                    std::optional<Options::UserMode>{Options::UserMode::NEXT};
            } else {
